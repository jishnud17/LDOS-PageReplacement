...
bool default_heuristic_policy(const page_stats_t *stats, migration_decision_t *decision) {
    if (stats == NULL || decision == NULL) return false;
    ...
    if (stats->current_tier == TIER_NVM && stats->heat_score > g_policy_config.hot_threshold) {
    ...
    if (stats->current_tier == TIER_DRAM && stats->heat_score < g_policy_config.cold_threshold) {
 ...
 */
bool predict_migration(const page_stats_t *stats, migration_decision_t *decision) {
...
static int execute_migration(migration_decision_t *decision) {
    ...
    TM_DEBUG("Migrated %p: %s -> %s (%s)", decision->page_addr,
             src->name, dest->name, decision->reason);
    return 0;
}
...
static void* policy_thread_loop(void *arg) {
    ...
    struct timespec sleep_time = { .tv_sec = 0, .tv_nsec = POLICY_INTERVAL_MS * 1000000L };
    ...
    while (g_manager.threads_running) {
        ...
        for (size_t i = 0; i < PAGE_STATS_HASH_SIZE && 
             migrations < g_policy_config.max_migrations_per_cycle; i++) {
            
            page_stats_t *entry = g_manager.page_stats_table[i];
            while (entry != NULL && migrations < g_policy_config.max_migrations_per_cycle) {
                ...
                if (predict_migration(entry, &decision) &&
                    ...
                    if (execute_migration(&decision) == 0) migrations++;
                    pthread_rwlock_rdlock(&g_manager.stats_lock);
                }
                entry = entry->next;
            }
        ...
        if (cycles % 100 == 0) {
            TM_INFO("Cycle %" PRIu64 ": pages=%" PRIu64 " faults=%" PRIu64 " migrations=%" PRIu64,
                    cycles,
...
int start_policy_thread(void) {
    ...
    if (pthread_create(&g_manager.policy_thread, NULL, policy_thread_loop, NULL) != 0) {
