...
int start_uffd_handler(void) {
    if (pthread_create(&g_manager.uffd_thread, NULL, uffd_handler_thread, NULL) != 0) {
...
int register_managed_region(void *addr, size_t length) {
    ...
    if (ioctl(g_manager.uffd, UFFDIO_REGISTER, &uffdio_register) < 0) {
        TM_ERROR("UFFDIO_REGISTER failed for %p+%zu: %s", addr, length, strerror(errno));
        pthread_mutex_unlock(&g_manager.regions_lock);
        return -1;
    }
    
    g_manager.regions[slot] = (managed_region_t){
        .base_addr = addr,
        .length = length,
        .uffd = g_manager.uffd,
        .active = true
...
static memory_tier_t decide_initial_placement(void *fault_addr) {
    ...
    if (dram->used + PAGE_SIZE <= dram->capacity) return TIER_DRAM;
    if (nvm->used + PAGE_SIZE <= nvm->capacity) return TIER_NVM;
...
static int resolve_page_fault(void *fault_addr, memory_tier_t tier) {
    ...
    if (ioctl(g_manager.uffd, UFFDIO_COPY, &uffdio_copy) < 0) {
        if (errno == EEXIST) return 0;  /* Race condition, harmless */
    ...
    for (int i = 0; i < MAX_MANAGED_REGIONS; i++) {
        ...
        if (r->active && page_addr >= r->base_addr && 
            ...
            if (tier == TIER_DRAM) atomic_fetch_add(&r->pages_in_dram, 1);
            else atomic_fetch_add(&r->pages_in_nvm, 1);
            break;
        }
    }
    pthread_mutex_unlock(&g_manager.regions_lock);
    ...
    TM_DEBUG("Resolved fault at %p -> %s", page_addr, tier == TIER_DRAM ? "DRAM" : "NVM");
    return 0;
}
